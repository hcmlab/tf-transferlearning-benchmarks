...
def read_image_lists(image_list):
    """
    Read tsv file with training, validation and test filenames and labels
    picturedir/classdir/filename\tclassname\t[training,validation,testing]
    classname should be lowercase and only contain alphanumeric ASCII letters
    """
    import csv

    if not tf.gfile.Exists(image_list):
        tf.logging.error("Image list '" + image_list + "' not found.")
        return None
    result = collections.OrderedDict()

    with open(image_list, newline='') as csvfile:
        ...
        for row in csvreader:
            ...
            if label not in result:
                ...
                result[label][type].append(filename)

    for label in result:
        for listtype in ["training", "testing", "validation"]:
            if not result[label][listtype]:
                tf.logging.warning('No files found for class \"{}\" in \"{}\"'.format(label, listtype))
                continue
            if len(result[label][listtype]) < 20:
                tf.logging.warning(
                    'WARNING: List for class \"{}\" in \"{}\" has less than 20 images, which may cause issues.'.format(label, listtype))
            elif len(result[label][listtype]) > MAX_NUM_IMAGES_PER_CLASS:
                tf.logging.warning(
                    'WARNING: List for class \"{}\" in \"{}\" has more than {} images. Some images will '
                    'never be selected.'.format(label, listtype, MAX_NUM_IMAGES_PER_CLASS))
...
def create_bottleneck_file(bottleneck_path, image_lists, label_name, index,
    ...
    log_file.write(datetime.now().isoformat() + "\tcreate_bottleneck_file\t" + "begin\n")
    ...
    log_file.write(datetime.now().isoformat() + "\tcreate_bottleneck_file\t" + "end\n")
...
def run_final_eval(sess, module_spec, class_count, image_lists,
    ...
    Args:
      ...
      bottleneck_tensor: The bottleneck output layer of the CNN graph.
    """
    log_file.write(datetime.now().isoformat() + "\tmodel_eval\t" + "begin\n")
    ...
    log_file.write(datetime.now().isoformat() + "\tmodel_test_acc_N\t" + '%.1f%%\t%d\n' % (test_accuracy * 100, len(test_bottlenecks)))
    log_file.write(datetime.now().isoformat() + "\tmodel_eval\t" + "end\n")

    confusion = tf.confusion_matrix(labels=test_ground_truth, predictions=predictions)
    with tf.Session():
        conf_out = tf.Tensor.eval(confusion, feed_dict=None, session=None)
    np.savetxt(log_file.name + "_eval.txt", conf_out)
...
def main(_):
    ...
    if not (FLAGS.image_dir or FLAGS.image_list):
        tf.logging.error('Must set flag --image_dir or --image_list.')
        return -1

    # Prepare necessary directories that can be used during training
    prepare_file_system()

    log_file.write(datetime.now().isoformat() + "\tsearching_picts\t" + "begin\n")

    image_lists = None

    # If we find image_list let's use the file ...
    # Format: relative file path, class, TESTING, VALIDATION
    if FLAGS.image_list:
        image_lists = read_image_lists(FLAGS.image_list)
    else:
    ...
    log_file.write(datetime.now().isoformat() + "\tsearching_picts\t" + "end\n")
    ...
    log_file.write(datetime.now().isoformat() + "\tmodel_loading\t" + "begin\n")
    # Set up the pre-trained graph.
    module_spec = hub.load_module_spec(FLAGS.tfhub_module)
    graph, bottleneck_tensor, resized_image_tensor, wants_quantization = (
        create_module_graph(module_spec))

    log_file.write(datetime.now().isoformat() + "\tmodel_loading\t" + "end\n")

    log_file.write(datetime.now().isoformat() + "\tmodel_adapt\t" + "begin\n")
    ...
    with tf.Session(graph=graph) as sess:
        ...
        log_file.write(datetime.now().isoformat() + "\tmodel_adapt\t" + "end\n")

        log_file.write(datetime.now().isoformat() + "\tbottlenecks_create_load\t" + "begin\n")
        ...
        log_file.write(datetime.now().isoformat() + "\tbottlenecks_create_load\t" + "end\n")
        ...
        log_file.write(datetime.now().isoformat() + "\ttrain\t" + "begin\n")
        ...
        for i in range(FLAGS.how_many_training_steps):
            ...
            if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
                train_accuracy, cross_entropy_value = sess.run(
                    ...
                    feed_dict={bottleneck_input: train_bottlenecks,
                               ground_truth_input: train_ground_truth})
                log_file.write(datetime.now().isoformat() + "\ttrain_step_trainacc_N\t" + '%d\t%.1f%%\t%d' % (
                i, train_accuracy * 100, len(train_bottlenecks)) + "\n")

                tf.logging.info('%s: Step %d: Train accuracy = %.1f%%' %
                                (datetime.now(), i, train_accuracy * 100))
                log_file.write(
                    datetime.now().isoformat() + "\ttrain_cross_entropy\t" + '%d\t%f' % (i, cross_entropy_value) + "\n")
                ...
                log_file.write(datetime.now().isoformat() + "\ttrain_step_valacc_N\t" + '%d\t%.1f%%\t%d' % (
        ...
        log_file.write(datetime.now().isoformat() + "\ttrain\t" + "end\n")
        ...
        log_file.write(datetime.now().isoformat() + "\tprogram\t" + "end\n")
        log_file.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default='',
        help='Path to file with image filenames, labels and type.'
    )
    parser.add_argument(
        '--output_graph',
        type=str,
    ...
    log_file = open(FLAGS.log_file_name, 'w')
    log_file.write(datetime.now().isoformat() + "\tprogram\t" + "begin\n")
